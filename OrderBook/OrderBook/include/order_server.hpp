...
class OrderServer {
public:
  OrderServer(ClientRequestLFQueue *client_requests, const std::string &iface,
              int port);
  ...
  /// Main run loop for this thread - accepts new client connections, receives
  /// client requests from them and sends client responses to them.
  auto run() noexcept {
  ...
  /// Read client request from the TCP receive buffer, check for sequence gaps
  /// and forward it to the FIFO sequencer.
    ...
    if (socket->next_rcv_valid_index_ >= sizeof(OMClientRequest)) {
      ...
      for (; i + sizeof(OMClientRequest) <= socket->next_rcv_valid_index_;
        ...
        if (cid_tcp_socket_[request->me_client_request_.client_id_] !=
          ...
          logger_.log("%:% %() % Received ClientRequest from ClientId:% on "
                      "different socket:% expected:%\n",
                      __FILE__, __LINE__, __FUNCTION__,
                      Common::getCurrentTimeStr(&time_str_),
        ...
        if (request->seq_num_ !=
            next_exp_seq_num) { // TODO - change this to send a reject back to
                                // the client.
          logger_.log("%:% %() % Incorrect sequence number. ClientId:% SeqNum "
                      "expected:% received:%\n",
                      __FILE__, __LINE__, __FUNCTION__,
                      Common::getCurrentTimeStr(&time_str_),
                      request->me_client_request_.client_id_, next_exp_seq_num,
                      request->seq_num_);
          continue;
        }
  ...
  /// End of reading incoming messages across all the TCP connections, sequence
  /// and publish the client requests to the matching engine.
...
private:
  ...
  /// Hash map from ClientId -> the next sequence number expected on incoming
  /// client requests.
