...
private:
  ...
  auto addOrdersAtPrice(MEOrdersAtPrice *new_orders_at_price) noexcept {
    ...
    const auto best_orders_by_price =
        (new_orders_at_price->side_ == Side::BUY ? bids_by_price_
                                                 : asks_by_price_);
  ...
  auto removeOrder(MEOrder *order) noexcept {
    ...
    cid_oid_to_order_.at(order->client_id_).at(order->client_order_id_) =
        nullptr;
  ...
  auto addOrder(MEOrder *order) noexcept {
    ...
    if (!orders_at_price) {
      ...
      auto first_order =
          (orders_at_price ? orders_at_price->first_me_order_ : nullptr);
