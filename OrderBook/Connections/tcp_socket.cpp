...
auto TCPSocket::sendAndRecv() noexcept -> bool {
  ...
  iovec iov{inbound_data_.data() + next_rcv_valid_index_,
  ...
  if (read_size > 0) {
    ...
    logger_.log("%:% %() % read socket:% len:% utime:% ktime:% diff:%\n",
                ...
                next_rcv_valid_index_, user_time, kernel_time,
  ...
  if (next_send_valid_index_ > 0) {
    ...
    logger_.log("%:% %() % send socket:% len:%\n", __FILE__, __LINE__,
                __FUNCTION__, Common::getCurrentTimeStr(&time_str_), socket_fd_,
                n);
  }
